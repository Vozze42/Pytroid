...
class Game_State():
        def __init__(self, level_reached = 0, points_total = 0, health = 0, asteroids_broken = 0, physics_manager = Physics_Manager()):
                self.level_reached = level_reached
                self.points_total = points_total
                self.health = health
                self.asteroids_broken = asteroids_broken  
                ...
                game_state = Game_State(physics_manager = physics_manager)
                Game_Object.game_state = game_state
        ...
        def update(self):
                while self.running:
                        ...
                        if max_asteroids > len(asteroids) and time_asteroid > asteroid_frequency:
...
def asteroidGenerator(number_ast, waveprop, leveltime, ): #set constant initially, increase if wave functionality added
        momentum = 400
